...
class FileTreeView extends StatelessWidget {
  ...
  void _showFileContextMenu(
    ...
    // Add plugin context menu items for files
    if (pluginManager != null && !data.isFolder) {
      // Create a minimal FileDocument for plugins
      // Note: In a real implementation, you might want to fetch the full FileDocument
      final fileDoc = FileDocument(
        id: data.id,
        name: data.name,
        folderId: data.parentId ?? '',
        content: '', // Empty content for context menu
      ...
      for (var i = 0; i < plugins.length; i++) {
        final pluginMenuItem = plugins[i].buildContextMenuItem(context, fileDoc);
        if (pluginMenuItem != null) {
          // Add divider before first plugin item
          if (i == 0) {
            menuItems.add(const PopupMenuDivider());
          }
          // Wrap plugin widget in PopupMenuItem
          menuItems.add(
            PopupMenuItem<String>(
              value: 'plugin_${plugins[i].manifest.id}',
              child: pluginMenuItem,
            ),
          );
        }
      }
