# Symbol Navigator Plugin - Architecture

## 🏗️ High-Level Architecture

```
┌────────────────────────────────────────────────────────────────────┐
│                         FLUTTER UI LAYER                            │
│  ┌──────────────────────────────────────────────────────────────┐  │
│  │           Symbol Navigator Plugin (Dart)                     │  │
│  │                                                              │  │
│  │  ┌─────────────────────────────────────────────────────┐    │  │
│  │  │ PRESENTATION                                        │    │  │
│  │  │  • SymbolNavigatorPlugin (BaseEditorPlugin)        │    │  │
│  │  │  • UI Descriptor Generation                        │    │  │
│  │  │  • Event Handlers (onFileOpen, onChange, onClose)  │    │  │
│  │  │  • State Management (StatefulPlugin)               │    │  │
│  │  └─────────────────────────────────────────────────────┘    │  │
│  │                           ↓↑                                 │  │
│  │  ┌─────────────────────────────────────────────────────┐    │  │
│  │  │ DOMAIN LAYER (Pure Business Logic)                 │    │  │
│  │  │                                                     │    │  │
│  │  │  Entities:                                          │    │  │
│  │  │  • CodeSymbol (name, kind, location, children)     │    │  │
│  │  │  • SymbolTree (symbols, statistics)                │    │  │
│  │  │                                                     │    │  │
│  │  │  Value Objects:                                     │    │  │
│  │  │  • SymbolKind (16 types: class, method, field...)  │    │  │
│  │  │  • SymbolLocation (line/column/offset tracking)    │    │  │
│  │  └─────────────────────────────────────────────────────┘    │  │
│  └──────────────────────────────────────────────────────────────┘  │
└────────────────────────────────────────────────────────────────────┘
                                 ↓↑
                    ╔═══════════════════════════╗
                    ║  WASM BRIDGE PROTOCOL     ║
                    ║                           ║
                    ║  • MessagePack            ║
                    ║  • Packed Pointers        ║
                    ║  • Memory Management      ║
                    ╚═══════════════════════════╝
                                 ↓↑
┌────────────────────────────────────────────────────────────────────┐
│                    GO WASM COMPUTATION LAYER                        │
│  ┌──────────────────────────────────────────────────────────────┐  │
│  │           Symbol Parser (Go + WASM)                          │  │
│  │                                                              │  │
│  │  ┌─────────────────────────────────────────────────────┐    │  │
│  │  │ WASM EXPORTS                                        │    │  │
│  │  │  • get_manifest() → PluginManifest                  │    │  │
│  │  │  • parse_symbols(ptr, len) → packed(ptr, len)      │    │  │
│  │  │  • alloc(size) → ptr                                │    │  │
│  │  │  • dealloc(ptr)                                     │    │  │
│  │  └─────────────────────────────────────────────────────┘    │  │
│  │                           ↓↑                                 │  │
│  │  ┌─────────────────────────────────────────────────────┐    │  │
│  │  │ PARSER LAYER                                        │    │  │
│  │  │                                                     │    │  │
│  │  │  Language Parsers:                                  │    │  │
│  │  │  • DartParser (regex-based, current)               │    │  │
│  │  │  • JavaScriptParser (TODO)                         │    │  │
│  │  │  • TypeScriptParser (TODO)                         │    │  │
│  │  │  • PythonParser (TODO)                             │    │  │
│  │  │  • GoParser (TODO: use stdlib go/parser)           │    │  │
│  │  │  • RustParser (TODO)                               │    │  │
│  │  │                                                     │    │  │
│  │  │  Future: Tree-sitter Integration                   │    │  │
│  │  │  • 10-100x faster                                  │    │  │
│  │  │  • 99.9% accuracy                                  │    │  │
│  │  └─────────────────────────────────────────────────────┘    │  │
│  └──────────────────────────────────────────────────────────────┘  │
└────────────────────────────────────────────────────────────────────┘
```

## 📐 Domain Model

### Entity-Relationship Diagram

```
┌─────────────────────────────────────────────────────────┐
│                     SymbolTree                          │
├─────────────────────────────────────────────────────────┤
│ + filePath: String                                      │
│ + language: String                                      │
│ + timestamp: DateTime                                   │
│ + parseDurationMs: int?                                 │
│ + symbols: List<CodeSymbol>                             │
├─────────────────────────────────────────────────────────┤
│ + getAllSymbols(): List<CodeSymbol>                     │
│ + findSymbolByName(name): CodeSymbol?                   │
│ + findSymbolAtLine(line): CodeSymbol?                   │
│ + statistics: Map<String, int>                          │
│ + totalCount: int                                       │
└─────────────────────────────────────────────────────────┘
                        1
                        │
                        │ contains
                        │
                        ▼ *
┌─────────────────────────────────────────────────────────┐
│                     CodeSymbol                          │
├─────────────────────────────────────────────────────────┤
│ + name: String                                          │
│ + kind: SymbolKind                                      │
│ + location: SymbolLocation                              │
│ + parentName: String?                                   │
│ + children: List<CodeSymbol>                            │
│ + metadata: Map<String, dynamic>                        │
├─────────────────────────────────────────────────────────┤
│ + isContainer: bool                                     │
│ + qualifiedName: String                                 │
│ + summary: String                                       │
│ + addChild(child): CodeSymbol                           │
│ + sortChildren(): CodeSymbol                            │
│ + getAllDescendants(): List<CodeSymbol>                 │
│ + findSymbolAtLine(line): CodeSymbol?                   │
└─────────────────────────────────────────────────────────┘
        │                           │
        │ has                       │ has
        ▼ 1                         ▼ 1
┌──────────────────┐      ┌─────────────────────────────┐
│   SymbolKind     │      │    SymbolLocation           │
├──────────────────┤      ├─────────────────────────────┤
│ • class          │      │ + startLine: int            │
│ • abstractClass  │      │ + startColumn: int          │
│ • mixin          │      │ + endLine: int              │
│ • extension      │      │ + endColumn: int            │
│ • enum           │      │ + startOffset: int          │
│ • typedef        │      │ + endOffset: int            │
│ • function       │      ├─────────────────────────────┤
│ • method         │      │ + lineCount: int            │
│ • constructor    │      │ + containsLine(line): bool  │
│ • getter         │      │ + containsOffset(int): bool │
│ • setter         │      └─────────────────────────────┘
│ • field          │
│ • property       │
│ • constant       │
│ • variable       │
│ • enumValue      │
│ • parameter      │
└──────────────────┘
```

## 🔄 Sequence Diagram - Parse Flow

```
User          Plugin         WASM Bridge      Go Parser      Tree-sitter
 │              │                 │               │               │
 │─ Open File ─▶│                 │               │               │
 │              │                 │               │               │
 │              │─ Schedule ─────▶│               │               │
 │              │   (debounce)    │               │               │
 │              │                 │               │               │
 │              │─ Detect Lang ──▶│               │               │
 │              │                 │               │               │
 │              │─ Serialize ────▶│               │               │
 │              │   MessagePack   │               │               │
 │              │                 │               │               │
 │              │                 │─ alloc() ────▶│               │
 │              │                 │               │               │
 │              │                 │─ parse_symbols()             │
 │              │                 │               │               │
 │              │                 │               │─ ParseSymbols()
 │              │                 │               │               │
 │              │                 │               │─ DartParser ─▶│
 │              │                 │               │   (regex)     │
 │              │                 │               │               │
 │              │                 │               │◀─ Symbols ────│
 │              │                 │               │   (5-10ms)    │
 │              │                 │               │               │
 │              │                 │◀─ packed(ptr,len)             │
 │              │                 │               │               │
 │              │◀─ Deserialize ──│               │               │
 │              │   SymbolTree    │               │               │
 │              │                 │               │               │
 │◀─ Update UI ─│                 │               │               │
 │   (tree)     │                 │               │               │
 │              │                 │               │               │
```

## 🧩 Component Diagram

```
┌─────────────────────────────────────────────────────────────────┐
│                        DART PLUGIN                              │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  ┌───────────────────────────────────────────────────────┐     │
│  │         SymbolNavigatorPlugin                         │     │
│  │         (BaseEditorPlugin + StatefulPlugin)           │     │
│  │                                                       │     │
│  │  • manifest: PluginManifest                           │     │
│  │  • currentSymbolTree: SymbolTree?                     │     │
│  │  • _disposed: bool                                    │     │
│  │  • _parseDebounce: Timer?                             │     │
│  │                                                       │     │
│  │  Methods:                                             │     │
│  │  + onInitialize(context)                              │     │
│  │  + onDispose()                                        │     │
│  │  + onFileOpen(file)                                   │     │
│  │  + onFileContentChange(id, content)                   │     │
│  │  + onFileClose(id)                                    │     │
│  │  + getUIDescriptor(): PluginUIDescriptor?             │     │
│  │  - _scheduleParseFile(file)                           │     │
│  │  - _parseFile(file)                                   │     │
│  │  - _detectLanguage(filename): String                  │     │
│  │  - _parseMockSymbols(content): List<CodeSymbol>       │     │
│  │  - _updateUI()                                        │     │
│  └───────────────────────────────────────────────────────┘     │
│                          │ uses                                 │
│                          ▼                                      │
│  ┌───────────────────────────────────────────────────────┐     │
│  │                   Domain Models                       │     │
│  │                                                       │     │
│  │  CodeSymbol ──▶ SymbolKind                            │     │
│  │      │          SymbolLocation                        │     │
│  │      └──▶ SymbolTree                                  │     │
│  └───────────────────────────────────────────────────────┘     │
└─────────────────────────────────────────────────────────────────┘
                          │ communicates via
                          ▼ MessagePack
┌─────────────────────────────────────────────────────────────────┐
│                        GO WASM PLUGIN                           │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  ┌───────────────────────────────────────────────────────┐     │
│  │                  main.go (Exports)                    │     │
│  │                                                       │     │
│  │  Exports:                                             │     │
│  │  • get_manifest() → uint64                            │     │
│  │  • parse_symbols(ptr, len) → uint64                   │     │
│  │  • alloc(size) → uint32                               │     │
│  │  • dealloc(ptr)                                       │     │
│  │                                                       │     │
│  │  Internal:                                            │     │
│  │  - packResult(data, err): uint64                      │     │
│  │  - readMemory(ptr, len): []byte                       │     │
│  └───────────────────────────────────────────────────────┘     │
│                          │ uses                                 │
│                          ▼                                      │
│  ┌───────────────────────────────────────────────────────┐     │
│  │            parser/parser.go (Dispatcher)              │     │
│  │                                                       │     │
│  │  + ParseSymbols(content, lang) → ([]Symbol, Duration) │     │
│  │  - parseDart(content): ([]Symbol, error)              │     │
│  │  - parseJavaScript(content): ([]Symbol, error)        │     │
│  │  - parseTypeScript(content): ([]Symbol, error)        │     │
│  │  - parsePython(content): ([]Symbol, error)            │     │
│  │  - parseGo(content): ([]Symbol, error)                │     │
│  │  - parseRust(content): ([]Symbol, error)              │     │
│  └───────────────────────────────────────────────────────┘     │
│                          │ implements                           │
│                          ▼                                      │
│  ┌───────────────────────────────────────────────────────┐     │
│  │          parser/dart_parser.go (Concrete)             │     │
│  │                                                       │     │
│  │  DartParser:                                          │     │
│  │  + Parse(): ([]Symbol, error)                         │     │
│  │  - parseClasses(): []Symbol                           │     │
│  │  - parseClassMembers(): []Symbol                      │     │
│  │  - parseMixins(): []Symbol                            │     │
│  │  - parseExtensions(): []Symbol                        │     │
│  │  - parseEnums(): []Symbol                             │     │
│  │  - parseTopLevelFunctions(): []Symbol                 │     │
│  │  - parseTopLevelVariables(): []Symbol                 │     │
│  │  - getLineNumber(offset): int                         │     │
│  │  - findMatchingBrace(pos): int                        │     │
│  │  - isInsideClass(offset): bool                        │     │
│  └───────────────────────────────────────────────────────┘     │
│                          │ uses                                 │
│                          ▼                                      │
│  ┌───────────────────────────────────────────────────────┐     │
│  │              parser/types.go (Data)                   │     │
│  │                                                       │     │
│  │  Symbol:                                              │     │
│  │  • Name, Kind, Location, Parent                       │     │
│  │  • Children: []Symbol                                 │     │
│  │  • Metadata: map[string]interface{}                   │     │
│  │                                                       │     │
│  │  Location:                                            │     │
│  │  • StartLine, StartColumn, EndLine, EndColumn         │     │
│  │  • StartOffset, EndOffset                             │     │
│  │                                                       │     │
│  │  Constants: KindClass, KindMethod, ...                │     │
│  └───────────────────────────────────────────────────────┘     │
└─────────────────────────────────────────────────────────────────┘
```

## 🔐 Memory Management

### WASM Memory Layout

```
┌─────────────────────────────────────────────────────────────┐
│                    WASM Linear Memory                       │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  Host (Dart) Side:                                          │
│  1. Serialize ParseRequest to MessagePack                   │
│  2. Call alloc(size) → get pointer                          │
│  3. Write MessagePack bytes to WASM memory at pointer       │
│  4. Call parse_symbols(ptr, len)                            │
│                                                             │
│  ┌────────────────────────────────────────────────────┐    │
│  │  Request Data (MessagePack)                        │    │
│  │  [content][language][filePath]                     │    │
│  │  ↑                                                  │    │
│  │  ptr (from alloc)                                   │    │
│  └────────────────────────────────────────────────────┘    │
│                                                             │
│  Go (WASM) Side:                                            │
│  5. Read bytes from WASM memory at [ptr:ptr+len]            │
│  6. Deserialize MessagePack to ParseRequest struct          │
│  7. Parse symbols with DartParser                           │
│  8. Serialize ParseResponse to MessagePack                  │
│  9. Allocate new memory for response                        │
│  10. Return packed (ptr << 32) | len                        │
│                                                             │
│  ┌────────────────────────────────────────────────────┐    │
│  │  Response Data (MessagePack)                       │    │
│  │  [symbols][language][duration][statistics]         │    │
│  │  ↑                                                  │    │
│  │  response ptr                                       │    │
│  └────────────────────────────────────────────────────┘    │
│                                                             │
│  Host (Dart) Side:                                          │
│  11. Unpack uint64 → (ptr, len)                             │
│  12. Read bytes from WASM memory at [ptr:ptr+len]           │
│  13. Deserialize MessagePack to ParseResponse               │
│  14. Convert to Dart SymbolTree                             │
│  15. (Optional) Call dealloc(ptr) for cleanup               │
│                                                             │
└─────────────────────────────────────────────────────────────┘

Packed Pointer Format (uint64):
┌─────────────────────────────────────────────────────────┐
│  Upper 32 bits: Pointer    │  Lower 32 bits: Length    │
│  [ptr ptr ptr ptr]          │  [len len len len]        │
└─────────────────────────────────────────────────────────┘
                OR
┌─────────────────────────────────────────────────────────┐
│  0xFFFFFFFFFFFFFFFF  →  Error (both ptr and len = max)  │
│  0x0000000000000000  →  Empty/Error (both zero)         │
└─────────────────────────────────────────────────────────┘
```

## 🎨 Design Patterns

### 1. **Plugin Pattern**
```dart
interface Plugin {
  PluginManifest manifest
  onInitialize(context)
  onDispose()
}

SymbolNavigatorPlugin implements Plugin, BaseEditorPlugin, StatefulPlugin
```

### 2. **Strategy Pattern** (Language Dispatch)
```go
func ParseSymbols(content, language) {
  switch language {
    case "dart": return parseDart(content)
    case "js":   return parseJavaScript(content)
    // ... more strategies
  }
}
```

### 3. **Composite Pattern** (Symbol Tree)
```dart
CodeSymbol {
  name
  kind
  location
  children: List<CodeSymbol>  // Recursive structure
}
```

### 4. **Observer Pattern** (StatefulPlugin)
```dart
StatefulPlugin {
  ValueNotifier<int> stateChanges
  setState(key, value) {
    _state[key] = value
    _notifier.value++  // Notify observers
  }
}
```

### 5. **Factory Pattern** (SymbolLocation)
```dart
factory SymbolLocation.fromTreeSitter({
  startRow, startCol, endRow, endCol, startByte, endByte
}) {
  return SymbolLocation(...)
}
```

### 6. **Builder Pattern** (PluginManifest)
```dart
PluginManifestBuilder()
  .withId('plugin.symbol-navigator')
  .withName('Symbol Navigator')
  .withVersion('0.1.0')
  .addActivationEvent('onFileOpen')
  .build()
```

## 📊 Performance Characteristics

### Time Complexity

| Operation | Current (Regex) | Future (Tree-sitter) |
|-----------|-----------------|----------------------|
| Parse 1K lines | O(n) ~5-10ms | O(n) ~0.5-1ms |
| Parse 10K lines | O(n) ~50-100ms | O(n) ~5-10ms |
| Find symbol by name | O(n) | O(1) with index |
| Find symbol at line | O(n) | O(log n) with BST |

### Space Complexity

| Structure | Size | Notes |
|-----------|------|-------|
| SymbolTree | O(n) | n = symbol count |
| CodeSymbol | O(1) | Fixed fields |
| WASM Request | O(m) | m = source length |
| WASM Response | O(n) | n = symbols |

## 🔄 State Management

```dart
SymbolNavigatorPlugin State Flow:

[File Opened] → [Debounce 500ms] → [Parse] → [Update State] → [Update UI]
     ↓                                  ↓
  setState('loading', true)      setState('symbolTree', tree)
     ↓                                  ↓
  getUIDescriptor()               setState('loading', false)
```

## 🎯 Extension Points

### Adding New Language Support

1. **Go side:**
```go
// parser/newlang_parser.go
func parseNewLang(content string) ([]Symbol, error) {
  // Implement parser
}

// parser/parser.go
case "newlang":
  symbols, err = parseNewLang(content)
```

2. **Dart side:**
```dart
// symbol_navigator_plugin.dart
String _detectLanguage(String filename) {
  switch (ext) {
    case 'newext': return 'newlang';
  }
}
```

### Adding New Symbol Kind

1. **Dart side:**
```dart
// symbol_kind.dart
const factory SymbolKind.newKind() = _NewKind;

int get iconCode {
  newKind: (_) => 0xXXXX,
}
```

2. **Go side:**
```go
// parser/types.go
const KindNewKind = "new_kind"
```

## 📚 References

- **Clean Architecture**: Domain/Infrastructure separation
- **DDD**: Entities (CodeSymbol), Value Objects (SymbolKind, Location)
- **SOLID**:
  - S: Each class has single responsibility
  - O: Open for extension (new languages)
  - L: Substitutable parsers
  - I: Minimal interfaces
  - D: Depend on abstractions (Parser interface)

## 🚀 Future Enhancements

1. **Incremental Parsing**: Only re-parse changed regions
2. **Semantic Analysis**: Type information, scopes
3. **Cross-references**: Find usages, go to definition
4. **Code Actions**: Refactorings based on symbols
5. **LSP Integration**: Use Language Server Protocol for accuracy
6. **Caching**: Cache parsed symbols, invalidate on change
7. **Indexing**: Build search index for fast symbol lookup
8. **Symbol Documentation**: Extract doc comments
